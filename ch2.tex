...
\begin{equation}
\text{AH} = \frac{{6.112 \times \exp(\left(\frac{{17.67 \times \text{T}\textsubscript{mean}}}{{\text{T}\textsubscript{mean} + 243.5}}\right) \times \text{RH}\textsubscript{mean} \times 2.1674}}){{273.15 + \text{T}\textsubscript{mean}}}
\label{eq:ah}
\end{equation}
...
We calculated a 90~m spatially distributed mean clear sky ONDJFM SWin estimate using the R package “insol”, which is based off of the Bird model (Bird & Hulstrom, 1981). The model uses the SNSR 90 m DEM as input and accounts for slope, aspect, differential shading, day of year, solar zenith angle, air temperature, elevation, and relative humidity. We held these values constant, with information on specific parametrization included in the supplement***. To calculate the mean ONDJFM SWin, average daily SWin values were calculated, and then a mean of the entire time series was taken. See
