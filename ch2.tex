...
Spatial and temporal scales of observations need to be relevant to the process. For instance, Bloschl showed that the relevant spatial scale for snow hydrology in mountain regions depends on ..... Need the right scales for measurements and models. 

\subsection{measurement scales and limitations of previous work}


\subsection{modeling scales and limitations of previous work}
Our early understanding of snowpack evolution came from 1-d models (Anderson 1976, Jordan (snow therm), marks 1999) but did not provide the capacity to understand topographic impacts on snowmelt. The next generation of physically-based energy balance models were spatially-explicit and incorporated topographic effects (alpine3d, isnobal, liston elder 2006). However, our empirical observations remain point-based and are physiographically limited. Common snow metrics like April 1st snow water equivalent (SWE) and maximum SWE (MSWE) are used to estimate spring flows. 

Yet, recent climate change is causing snow droughts and midwinter dry spells \citep{harpoldDefiningSnowDrought2017, hatchettMidwinterDrySpells2023}. Many mountain regions are experiencing midwinter melt events, thus impacting MSWE and related melt season processes. Previous work in snowpack modeling have demonstrated that an approximately 100~m grid scale is needed to characterize energy balance processes that govern snowpack evolution (winstral et al. 2014, sproles 2015, others). 


However, previous data limitations have made it difficult to explicitly investigate ... what is need *****

%% gaps we're addressing re: pillows and big models
\subsection{previous work and gaps}

While a variety of past studies have investigated, 
...
%  modeling
\cite{pomeroyVariationSurfaceEnergetics2003} used in situ meteorologic instrumentation setup slope normal, instead of the traditional gravimetric orientation, and found marked differences in incoming shortwave radiation (SW$_{in}$) and ablation rates on south and north facing slopes (**rework**). 

% 
modeling** \citep{mazzottiCanopyStructureTopography2023, lopez-morenoEffectSlopeAspect2014}. 

***While these studies use spatially distributed information, there temporally scale is limited from weeks to a few snow seasons over smaller spatial*** 
...
These physiographic factors impact the local energy balance dynamics, which govern the accumulation and ablation of the snowpack. 
 ...
 \citep{tennantRegionalSensitivitiesSeasonal2017, molotchEcohydrologicalControlsSnowmelt2009} 
