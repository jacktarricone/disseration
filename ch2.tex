...
The USJ is approximately 4244~km$^{2}$ in land area, spanning from the high-elevation peaks (4227~m) in the east to flat low-elevation (153~m) Central Valley in the west (Fig.~\ref{fig:kuy_study_area}d). The main channel flows into Millerton Lake, which is one of the largest reservoirs in the Central Valley. 
...
SWE data from 29 SNOTEL sites (black dots in Fig.~\ref{kuy_study_area}a) between 1985--2016, totaling $\sim$840 station years, were used for snow metric validation. Operated by the United States Department of Agriculture's Natural Resources Conservation Service (NRCS), it is a comprehensive network of remote, long-term, automated snowpack and climate measurement sites spread across the western United States and Alaska. These data are quality controlled by the NRCS and are freely available.

\hypertarget{ch2-do-2}{\subsubsection{gridMET Data}\label{ch2-do-2}}

To understand the physical mechanisms driving snowpack metric variation, we used 4-km meteorological data from gridMET \citep{abatzoglouDevelopmentGriddedSurface2013} to calculate annual (1985--2016) cold season values from October 1 to March 31 (ONDJFM) for three meteorologic variables: mean temperature (T\textsubscript{mean}), relative humidity (RH\textsubscript{mean}), and incoming solar radiation (insolation). Daily ONDJFM were averaged to create the annual metrics. These data were bilinearly downscaled to the 90~m SNSR resolution. Since gridMET does not include a T\textsubscript{mean} or RH\textsubscript{mean} value --- only the daily maximum and minimum --- these values were averaged to create the two mean variables. Insolation from gridMET is estimated with respect to a planar surface and not topographically corrected. 

%===========================================================================
\hypertarget{ch2-do-2}{\subsubsection{Clear sky insolation}\label{ch2-do-2}}

To look at aspect-driven variations in incoming solar radiation, 90-m spatially distributed daily mean clear sky incoming solar radiation (CS insolation) values were calculated in ($\mathrm{W~m}^{-2}$). These values were then averaged over ONDJFM time period in the same fashion as the gridMET data. Calculations were performed using the R package ``insol” \citep{corripioInsolSolarRadiation2020}, which is based on the Bird model \citep{birdReviewEvaluationImprovement1981}. The R insol model uses a 90-m DEM as input and accounts for slope, aspect, differential shading, day of year, and solar zenith angle. 
Singular values for air temperature, elevation, and relative humidity were used to parameterize the model. Note that CS insolation does not account for cloud cover or variations in latitude. Thus, we produce a gridded estimate of CS insolation that represents the average cold-season clear sky radiation for any year (i.e., not a time series).
...
These metrics were then validated against data from 29 SNOTEL stations totaling $\sim$840 station years (Figure \ref{kuy_study_area}). SNSR values were selected by the specific pixel in which the SNOTEL station fell. Two correlation coefficients (R and R$^{2}$), root-mean-square error (RMSE), mean absolute error (MAE), mean error (ME), and percent bias (PB) are in Table \ref{tab:snow_metrics_val_table}.
